...
void logorsignin(Login *a) {
  ...
  switch (login) {
    ...
    case 2:
      ...
      scanf("%s", lname);
      int test=0;
      //Loop to make sure every user has a diffirent id
      do{
      printf("Choose an id\n");
      scanf("%s", sid);
      ...
      if (fichier_signup == NULL) {
        ...
        fprintf(fichier_signup, "\n%s %s %s %s %d", name, lname, sid, spassword, a->mode);
...
void reset(){
  int reset;
  do{
  printf("Do you want to buy the articles?\n");
  printf("1. Yes\n");
  printf("2. No\n");
  scanf("%d",&reset);
  ...
  switch(reset){
    case 1 :
      printf("You have buy the articles\n");
    FILE*rese=fopen("buy.txt","w");
if (rese == NULL) {
...
int buying(){
 ...
 while (fscanf(buyingcart, " %s %d %f", a.name, &a.ref_num, &a.price) == 3) {
  ...
  printf("Here are the article that you want to buy\n\n");
  rewind(buyingcart);
  for(i=0;i<num;i++){
